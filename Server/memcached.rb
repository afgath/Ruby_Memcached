...
class Memcached
  attr_accessor :port
  attr_reader :hostname, :server, :highest_id, :cache
          ...
          if array_values.count < 4 || array_values.count > 5 || !valid
            ...
            if valid
              if array_values.count == 4 # If item hasn't set the NoReply attribute
                item = Item.new(array_values[1], array_values[2], array_values[3], '', nil , nil, nil)
              elsif array_values.count == 5 # If item has set the NoReply attribute
                item = Item.new(array_values[1], array_values[2], array_values[3], array_values[4], nil , nil, nil)
              end
              if apply
